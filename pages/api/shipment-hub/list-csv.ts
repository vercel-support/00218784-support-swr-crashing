...
const explodeHubDataByCost = (BoLHubs: any) => {
  const costBasedData = BoLHubs.map((BoLHub: any) => {
    ...
    const subtotalCost = calculateSubotalCost(BoLHub?.costs?.costs)
    const hubMargin = calculateMargin(subtotalPrice, subtotalCost, 4)
    const locationsLenght = BoLHub?.locations?.locations.length || 0
    
    // console.log(BoLHub)
    // console.log('hubMargin', hubMargin)
    // console.log('BoLHub?.costs', BoLHub?.costs)
    // console.log('BoLHub?.costs.leght', BoLHub?.costs.length)
    let costBasedRows: any[] = []
    if (BoLHub?.costs?.costs && BoLHub?.costs?.costs.length > 0 && typeof BoLHub?.costs?.costs !== 'undefined') {
      costBasedRows = BoLHub?.costs?.costs?.map((cost: any) => {
        ...
        // console.log(BoLHub?.transports[cost?.transportIndex || 0].figures)
        ...
        const costBasedRow = {
          ...
          pickupDate: BoLHub?.locations?.locations[0] ? BoLHub?.locations?.locations[0].departureArrivalDateTime?.print : 'N/A',
          pickupCompany: BoLHub?.locations?.locations[0] ? BoLHub?.locations?.locations[0].company?.name : 'N/A',
          pickupPlace: BoLHub?.locations?.locations[0] ? BoLHub?.locations?.locations[0].place?.formattedAddress : 'N/A',
          deliveryDate: BoLHub?.locations?.locations[lastLocationIndex]
            ? BoLHub?.locations?.locations[lastLocationIndex].departureArrivalDateTime?.print
            : 'N/A',
          deliveryCompany: BoLHub?.locations?.locations[lastLocationIndex]
            ? BoLHub?.locations?.locations[lastLocationIndex].company?.name
            : 'N/A',
          deliveryPlace: BoLHub?.locations?.locations[lastLocationIndex]
            ? BoLHub?.locations?.locations[lastLocationIndex].place?.formattedAddress
            : 'N/A',
          carrier: cost?.company?.comercialName || 'N/A',
          vehicleType: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.vehicle?.typeOfVehicleSAT || 'N/A',
          vehicleNumber: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.vehicle?.number || 'N/A',
          vehiclePlateNumber: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.vehicle?.plateNumber || 'N/A',
          trailerType: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.trailers?.typeOfTrailerSAT || 'N/A',
          trailerNumber: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.trailers?.number || 'N/A',
          trailerPlateNumber: BoLHub?.transports?.transports[cost?.transportIndex || 0].autotransport?.trailers?.plateNumber || 'N/A',
          operatorName:
            cost?.transportIndex && BoLHub?.transports?.transports[cost?.transportIndex || 0].figures
              ? BoLHub?.transports?.transports[cost?.transportIndex || 0].figures[0].name
              : 'N/A',
          pedimentos: getPedimentosInString(BoLHub?.goods?.good) || 'N/A',

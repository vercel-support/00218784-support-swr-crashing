...
export const routeHandler = async (
  ...
  const { body: vehicle, dbService, loggedUser }: { body: any; dbService: DbService; loggedUser: User } = req
  if (!vehicle) return next(new RequestError(400, 'saveNewPlace.errors.noData'))
  // console.log('vehicle in api:', vehicle, 'loggedUser in api', loggedUser)
  await dbService.saveNewVehicle(vehicle, loggedUser._id)
  res.json({ ok: true })
  return next()
}

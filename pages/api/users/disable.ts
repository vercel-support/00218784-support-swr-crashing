...
export const routeHandler = async (req: Request, res: NextApiResponse, next: Function) => {
  ...
  // console.log(userId)
  // refactor setUserDisabled to only receive userId and not logged User
  await dbService.setUserDisabled(user._id)
  res.json({ ok: true })
  const eventDetails = { userId: user._id, username: user.username, userLastName: user.userLastName || null }
  await dbService.addEventLog(dbService.eventLogTypes.userDisable, eventDetails, loggedUser)
  return next()
}

...
export const NavigationMenu = ({ hubState, hubDispatch, currentTab, setCurrentTab, isPart}) => {
  ...
  //     },
  //     {
  //       label: i18n('shipmentSections.locations'),
  //       icon: "",
  //       key: 'locations',
  //     },
  //     {
  //       label: i18n('shipmentSections.load'),
  //       icon: "",
  //       key: 'load',
  //     },
  //     {
  ...
  const itemsFiltrados = items.filter(item => hubState?.activeSections?.includes(item.key));
  const detailsItem = items.find(item => item.key === 'details');

  if (detailsItem) {
    itemsFiltrados.unshift(detailsItem);
  }
  ...
  return (
    <Affix offsetTop={top}>
      <Menu onClick={onClick} selectedKeys={[currentTab]} mode="horizontal" items={itemsFiltrados} className="mt-0 px-2" />

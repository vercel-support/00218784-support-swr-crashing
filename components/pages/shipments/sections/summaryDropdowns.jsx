...
import { Badge, Dropdown, Tag, Timeline, Button, Form, Input, Radio, Space, Card, Spin, Select } from 'antd'
import { SmileOutlined, ClockCircleOutlined, CloseCircleOutlined, CheckCircleOutlined, ExclamationCircleOutlined } from '@ant-design/icons'
import { i18n } from '../../../../services/i18n'
import { WantToAddComment } from '../tabs/wantToAddComment'
import { dateFormat } from '../../../../services/helpers/dateFormat'
import useSWR from 'swr'
import { post, get } from '../../../../services/fetch/index.js'
import { saveChangeInDB } from '../actions/shipmentActions.jsx'
import { loadCategories, modeTypes, optionTypes, recurrenceTypes, servicesTypes, tripTypes, urgencyTypes } from '../../../../services/catalogs-shipment/index.js'
...
export const MilestonesTimeLine = ({milestones, stages}) => {
  let prevChildrenTrue = false;
  let items = [];

  const milestonesStatuses = {
    requested: ['requested', 'acceptedByCustomer', 'rejectedByCustomer', 'canceledByCustomer'],
    booking: ['booking', 'schedulingVehicle', 'confirmedVehicle', 'noVehicleAvailable'],
    pickup: ['pickup', 'inRouteToPickup', 'arrivingToPickup', 'startingToLoad', 'finishingLoading', 'pickupProblems'],
    inTransit: ['inTransit', 'waiting', 'waitingDocuments', 'waitingAppointment', 'driverOnABreak', 'accident', 'delay', 'incident', 'unknownETA', 'arrivingToDelivery'],
    delivery: ['delivery', 'arrivingToDelivery', 'startingToUnload', 'finishingUnloading', 'delivered', 'deliveryProblems'],
  };

  const arraymilestones = ["requested", "booking", "pickup", "inTransit", "delivery"]
  ...
  const statusColor = {
    ...
    booking: '#0B58A8',
    schedulingVehicle: '#0B58A8',
    confirmedVehicle: 'green',
    noVehicleAvailable: '#f5c723',
    pickup: '#0B58A8',
    inRouteToPickup: '#0B58A8',
    arrivingToPickup: '#0B58A8',
    startingToLoad: '#0B58A8',
    ...
    startingToUnload: '#0B58A8',
    finishingUnloading: '#0B58A8',
    delivered: 'green',
  ...
  milestones?.event?.forEach((evento) => {
    const status = evento?.status;
    const stage = evento?.name;
    const plannedDate = evento?.plannedDate;
    const completedDate = evento?.completedDate;

    if (milestonesStatuses[stage]?.includes(stage)) {
      ...
      items.push({
        ...
        children: (
          <>
            <p style={{ fontWeight: 'bold', fontSize: '.8rem' }}>{stage}</p>
            <p>{status}</p>
            <p style={{fontSize: '.55rem' }}>{plannedDate && new Date(plannedDate).toLocaleDateString()}</p>
            <p style={{fontSize: '.55rem' }}>{completedDate && new Date(completedDate).toLocaleDateString()}</p>
          </>
        ),
      })

      prevChildrenTrue = color === 'green';
    }
  });
...
export const SummaryInfo = ({ shipmentState }) => {
  const { data: creator } = useSWR(`/api/shipment/get-user-byid?id=${shipmentState.createdBy}`, post, {
    dedupingInterval: 3000,
  })

  const { data: manager } = useSWR(`/api/shipment/get-user-byid?id=${shipmentState.managedBy}`, post, {
    dedupingInterval: 3000,
  })

  const listUrl = `/api/users/list`
  const { data: usersData, error } = useSWR(listUrl, url => post(url, {}))
  ...
  console.log('clientsData', clientsData?.clients);
  // if (error) return <div>{error}</div>
  // if (!data) return <div>No data</div>
  // if (data.error) return <div>{data.error}</div>
  // if (data.users.length === 0) return <div>Empty state</div>

  // if (error) return <div>client error: {error}</div>
  // if (!data || !data.ok)
  //   return (
  ...
  // console.log('data', creator, manager)
  // console.log('username', username);

  const onChange = (fieldName, selectedValue, fieldName2, nameValue) => {
    // console.log('Nombre del campo:', fieldName);
    // console.log('Valor seleccionado:', selectedValue);
    // console.log('nameValue:', nameValue);
    // console.log('fieldName2:', fieldName2);
    saveChangeInDB(shipmentState._id, 'Update', { [fieldName]: selectedValue })

    if(fieldName2 && nameValue ){
      saveChangeInDB(shipmentState._id, 'Update', { [fieldName2]: nameValue })
    }
  ...
  useEffect(() => {
    form.setFieldsValue({
      ...
      managedBy: manager?.username,
      client: shipmentState.clientName,
      load: shipmentState.loadType,
      trip: shipmentState.tripType,
      mode: shipmentState.modeType,
      service: shipmentState.serviceType,
      recurrence: shipmentState.recurrenceType,
      urgency: shipmentState.urgencyType,
      opcion: shipmentState.optionType,
    })
  }, [creator, manager, shipmentState ]);
    ...
    >
      ...
      <Form.Item name="createdBy" label="Created By">
        <Select
          ...
          style={{ width: '100'}}
          options={usersData?.users?.map(({ _id, username }) => ({ value: _id, label: username }))}
        >
        </Select>
      </Form.Item>
      <Form.Item name="managedBy" label="Manager">
        {/* <Input placeholder="input placeholder" value={manager?.username} /> */}
        <Select
          showSearch
          onChange={value => onChange ('managedBy', value )}
      ...
      <Form.Item label="Create At">
        <Input placeholder="input placeholder" value={shipmentState.createdAt} readOnly  />
      </Form.Item>
      <Form.Item label="Folio">
        <Input placeholder="input placeholder" value={shipmentState.folio} readOnly/>
      </Form.Item>
      <Form.Item name="client" label="Client">
        <Select
          showSearch
          onChange={(value, option) => onChange ('clientId', value, 'clientName' ,option.label )}
          placeholder="Client"
          filterOption={(input, { label }) => label.toLowerCase().includes(input.toLowerCase())}
          style={{ width: '100'}}
      ...
      <Form.Item name="load" label="Load">
        <Select
          ...
          style={{ width: '100'}}
          options={loadCategories?.map(({name }) => ({ value: name, label: i18n(`shipment.loadCategories.${name}`) }))}
        >
        </Select>
      </Form.Item>
      <Form.Item name='trip' label="Trip">
        <Select
          onChange={(value) => onChange ('tripType', value)}
          placeholder="Trip type"
          style={{ width: '100'}}
          options={tripTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.tripTypes.${name}`) }))}
        >
        </Select>
      </Form.Item>
      <Form.Item name='mode' label="Mode">
        <Select
          onChange={(value) => onChange ('modeType', value)}
          placeholder="Mode type"
          style={{ width: '100'}}
          options={modeTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.modeTypes.${name}`) }))}
        >
        </Select>
      </Form.Item>
      <Form.Item name="service" label="Service">
        <Select
          ...
          style={{ width: '100'}}
          options={servicesTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.servicesTypes.${name}`) }))}
        ></Select>
      </Form.Item>
      <Form.Item name="recurrence" label="Recurrence">
        <Select
          ...
          style={{ width: '100'}}
          options={recurrenceTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.recurrenceTypes.${name}`) }))}
        ></Select>
      </Form.Item>
      <Form.Item name="urgency" label="Urgency">
        <Select
          onChange={(value) => onChange ('urgencyType', value)}
          placeholder="Urgency type"
          style={{ width: '100'}}
          options={urgencyTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.urgencyTypes.${name}`) }))}
        ></Select>
      </Form.Item>
      <Form.Item name="opcion" label="Option">
        <Select
          onChange={(value) => onChange ('optionType', value)}
          placeholder="Option type"
          style={{ width: '100'}}
          options={optionTypes?.map(({name }) => ({ value: name, label: i18n(`shipment.optionTypes.${name}`) }))}
        ></Select>
      </Form.Item>
      {/* <Form.Item label="Status">
        <Input placeholder="input placeholder" value={i18n(`shipmentStatusMenu.${shipmentState?.operativeStatus?.text}`)} />
      </Form.Item>
      <Form.Item label="Collect">
        <Input placeholder="input placeholder" value={i18n(`shipmentStatusMenu.${shipmentState?.collectStatus?.text}`)} />
      </Form.Item>
      <Form.Item label="Supplier">
        <Input placeholder="input placeholder" value={i18n(`shipmentStatusMenu.${shipmentState?.supplierStatus?.text}`)} />
      </Form.Item> */}

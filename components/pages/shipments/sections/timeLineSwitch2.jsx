...
const TimelineComponent2 = ({ milestones, hubState}) => {
  ...
  return (
    <div className='mt-2'>
      <Switch checkedChildren="Detailed View" unCheckedChildren="Default View" onChange={setDetailedView} />
      <Timeline mode="left" className='mt-4'>
        {getStageMilestones().map(({ key, label, status, detailedData }) => {
          ...
          return (
            <React.Fragment  key={key} color={color} dot={dot} >
            {detailedView ? (
              <>
                <Timeline.Item key={key} color={color} dot={dot}><p style={{ fontWeight: 'bold', fontSize: '.9rem' }}>{label}</p></Timeline.Item>
            ...
            <Timeline.Item key={key} color={color} dot={dot}>
              <>
                <p style={{ fontWeight: 'bold', fontSize: '.8rem' }}>{label}</p>
                <p>{status}</p>
                {detailedData && (
                  <>
                    <p>{detailedData.name} - {detailedData.status}</p>
                  </>
                )}
              </>
            </Timeline.Item>

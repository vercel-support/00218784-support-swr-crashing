...
import { EyeOutlined, FlagOutlined, FlagTwoTone } from '@ant-design/icons'
...
const InvoiceListPageLoadingState = ({ scope, handleTabClick }) => {
  const tabsItemsLoading = [
    {
      key: 'internal',
      label: i18n('shipment.myCompanyShipments'),
      children: <Empty image={<LoadingOutlined className="p-24 text-6xl" />} description="" />,
    },
    {
      key: 'external',
  ...
  return (
    <div className="bg-white w-full mb-0 mt-4">
      <Tabs defaultActiveKey={scope} items={tabsItemsLoading} className="mx-4 pt-4" onTabClick={handleTabClick} />
    </div>
  )
}

const InvoiceListPageEmptyState = ({ scope, handleTabClick }) => {
  const tabsItemsNoData = [
    {
      key: 'internal',
      label: i18n('shipment.myCompanyShipments'),
      children: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span>{i18n('billing.emptyPageDescription')}</span>} />,
    },
    {
      key: 'external',
      label: i18n('shipment.externalShipments'),
  ...
  return (
    <div className="bg-white w-full mb-0 mt-4">
      <Tabs defaultActiveKey={scope} items={tabsItemsNoData} className="mx-4 pt-4" onTabClick={handleTabClick} />
    </div>
  )
}
...
export const ShipmentList = ({
  ...
  // if (!data) return <InvoiceListPageLoadingState scope={scope} handleTabClick={handleTabClick} />
  // if (data.error) return <InvoiceListPageErrorState errorMessage={data.error} />
  // if (data?.shipments?.length === 0) return <InvoiceListPageEmptyState scope={scope} handleTabClick={handleTabClick} />
  ...
  const dataForTable = data?.shipments?.map(shipment => {
    ...
    return {
      ...
      origin: {
        city: origins?.length > 0 ? origins[0].place?.satAddress?.locality?.satDescription || origins[0].place?.satAddress?.locality : '',
        state:
          origins?.length > 0 ? origins[0].place?.satAddress?.politicalState?.satCode || origins[0].place?.satAddress?.politicalState : '',
        country: origins?.length > 0 ? origins[0].place?.satAddress?.country : '',
        date: origins?.length > 0 ? origins[0].date : '',
      },
      destination: {
  ...
  const defineContent = (data, columns) => {
    if (!data) return <Empty image={<LoadingOutlined className="p-8 text-2xl" />} description="" />
    if (data?.length === 0) return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<span>{i18n('billing.emptyPageDescription')}</span>} />
    return (
      <Table
  ...
  const tabsItems = [
    {
      key: 'internal',
      label: i18n('shipment.myCompanyShipments'),
      children: defineContent(data, columnsForInternalUsers)
      // children: (
      //   <>
      //     <Table
      //       // rowSelection={rowSelection}
    ...
    },
    {
      key: 'external',
      label: i18n('shipment.externalShipments'),
      children: defineContent(data, columnsForExternalUsers)
      // children: (
      //   <Table
      //     // rowSelection={rowSelection}

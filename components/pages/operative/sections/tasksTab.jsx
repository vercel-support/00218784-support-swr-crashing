...
export const TasksTab = ({ hubState, hubDispatch, getViewPermissions, getWritingPermissions, handleSubmit, onSubmitLoad, loggedEmail }) => {
  ...
  return (
    <>
      <Card className="w-full mt-0 border-l-8 border-t-0 border-b-0 border-r-0 border-tkyBlue" id="quotes">
        <Form layout="vertical" onFinish={handleSubmit(onSubmitLoad)}>
          <Text className="text-lg font-medium">{i18n(`newBillOfLadingHub.tasks.title`)}</Text>
          <div className="mt-4">{i18n('newBillOfLadingHub.tasks.description')}</div>
